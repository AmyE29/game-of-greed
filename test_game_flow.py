import pytest
from game import Game

###############################################
#####  Day 1                             ######
###############################################

@pytest.mark.skip('superceded by later day test')
def test_flow_yes():

    prints = ['Welcome to the Game of Greed','Check back tomorrow :D']
    prompts = ['Wanna play (y/n)?']
    responses = ['y']

    def mock_print(*args):
        if len(prints):
            current_print = prints.pop(0)
            assert args[0] == current_print

    def mock_input(*args):
        if len(prompts):
            current_prompt = prompts.pop(0)
            assert args[0] == current_prompt

        if len(responses):
            current_response = responses.pop(0)
            return current_response

    game = Game(mock_print, mock_input)

    game.play()

def test_flow_no():

    flow = {
        'prints' : ['Welcome to the Game of Greed','OK. Maybe another time'],
        'prompts' : ['Wanna play (y/n)?'],
        'responses' : ['no'],
    }

    mp = MockPlayer(**flow)

    game = Game(mp.mock_print, mp.mock_input)

    game.start_game = mp.mock_roll

    game.play()

    assert mp.mop_up()

###############################################
#####  Day 2                             ######
###############################################
def test_one_round():
    flow = {
        'prints' : [
            'Welcome to the Game of Greed',
            'Rolling 6 dice',
            'You rolled [1, 2, 2, 3, 3, 4]',
            'Do you want to bank your score of 100 or press your luck for more points?',
            'You have 5 dice remaining',
            'Rolling 5 dice',
            'You rolled [1, 2, 2, 3, 3]',
            'Do you want to bank your score of 200 or press your luck for more points?',
            'You have 4 dice remaining',
            'Rolling 4 dice',
            'You rolled [1, 2, 2, 3]',
            'Do you want to bank your score of 300 or press your luck for more points?',
            'You have 3 dice remaining',
            'Rolling 3 dice',
            'You rolled [1, 2, 2]',
            'Do you want to bank your score of 400 or press your luck for more points?',
            'You have 2 dice remaining',
            'Rolling 2 dice',
            'You rolled [1, 2]',
            'Do you want to bank your score of 500 or press your luck for more points?',
            'You have 1 dice remaining',
            'You banked 500 points in round 1',
            'Whoo hoo! You have 500 points so far',
            'Come play again soon'
        ],
        'prompts' : [
            'Wanna play (y/n)?',
            'Enter dice to keep: ',
            'Roll again? ',
            'Enter dice to keep: ',
            'Roll again? ',
            'Enter dice to keep: ',
            'Roll again? ',
            'Enter dice to keep: ',
            'Roll again? ',
            'Enter dice to keep: ',
            'Roll again? ',
        ],
        'responses' : [
            'y','1','y','1','y','1','y','1','y','1','n'
        ],
        'rolls' : [
            [1, 2, 2, 3, 3, 4],
            [1, 2, 2, 3, 3],
            [1, 2, 2, 3],
            [1, 2, 2],
            [1, 2],
        ]
    }

    mp = MockPlayer(**flow)

    game = Game(mp.mock_print, mp.mock_input)

    game._start_game = mp.mock_roll

    game.play(1)

    assert mp.mop_up()

def test_flow_scenario_1():

    flow = {
        'prints' : [
            'Welcome to the Game of Greed',
            'Rolling 6 dice',
            'You rolled [1, 2, 3, 4, 1, 2]',
            'Do you want to bank your score of 100 or press your luck for more points?',
            'You have 5 dice remaining',
            'Rolling 5 dice',
            'You rolled [3, 3, 3, 4, 1]',
            'Do you want to bank your score of 500 or press your luck for more points?',
            'You have 1 dice remaining',
            'You banked 500 points in round 1',
            'Whoo hoo! You have 500 points for far',
            'Come play again soon!'
        ],

        'prompts' : [
            'Wanna play (y/n)?',
            'Enter dice to keep: ',
            'Roll again? '
        ],

        'responses' : ['y','1','y','3331','n'],

        'rolls' : [[1,2,3,4,1,2],[3,3,3,4,1]],
    }

    mp = MockPlayer(**flow)

    game = Game(mp.mock_print, mp.mock_input)

    game._start_game = mp.mock_roll

    game.play(1)

    assert mp.mop_up()

def test_flow_scenario_2():

    flow = {
        'prints' : [
            'Welcome to the Game of Greed',
            'Rolling 6 dice',
            'You rolled [1, 1, 1, 1, 5, 2]',
            'Do you want to bank your score of 2050 or press your luck for more points?',
            'You have 1 dice remaining',
            'You banked 2050 points in round 1',
            'You have 2050 points total',
            'Come play again soon!',
        ],

        'prompts' : [
            'Wanna play (y/n)?',
            'Enter dice to keep: ',
            'Roll again? '
        ],

        'responses' : ['y','11115','n'],

        'rolls' : [[1,1,1,1,5,2],],
    }

    mp = MockPlayer(**flow)

    game = Game(mp.mock_print, mp.mock_input)

    game._start_game = mp.mock_roll

    game.play(1)

    assert mp.mop_up()

def test_flow_farkle():

    flow = {
        'prints' : [
            'Rolling 6 dice',
            'You rolled [2, 2, 3, 4, 6, 6]',
            'FARKLE',
        ],
        'rolls' : [[2,2,3,4,6,6]],
    }

    mp = MockPlayer(**flow)

    game = Game(mp.mock_print, mp.mock_input)

    game._start_game = mp.mock_roll


    game.plat_game()

    assert mp.mop_up()


###############################################
#####  Day 3 - Coming Soon               ######
###############################################
def test_validate_selected_dice():
    """"
    add a test to confirm that user's selected
    "keeper" dice are a valid subset of the user's roll
    """
    pass

def test_farkle_ends_round():
    """"
    add a test to confirm that a farkle roll
    ends the turn and no points are awarded
    """
    pass




###############################################
#####  Helper Class for Testing          ######
###############################################
class MockPlayer:
    def __init__(self, prints=[], prompts=[], responses=[], rolls=[]):
        self.prints = prints
        self.prompts = prompts
        self.responses = responses
        self.rolls = rolls

    def mock_print(self, *args):
        if len(self.prints):
            current_print = self.prints.pop(0)
            assert args[0] == current_print

    def mock_input(self, *args):
        if len(self.prompts):
            current_prompt = self.prompts.pop(0)
            assert args[0] == current_prompt

        if len(self.responses):
            current_response = self.responses.pop(0)
            return current_response

    def mock_roll(self, num_dice):
        if len(self.rolls):
            current_roll = self.rolls.pop(0)
            return current_roll

    def mop_up(self):
        assert len(self.prints) == 0
        assert len(self.prompts) == 0
        assert len(self.responses) == 0
        assert len(self.rolls) == 0
        return True
